...
const assignmentStatus = document.getElementById("assignment_status");
const dueDate = document.getElementById("due_date");
const notes = document.getElementById("assignment_notes");

const cancelSeamstressEditBtn =
  document.getElementById("cancelSeamstressEditBtn");
  ...
  seamstressSelect.innerHTML =
    ...
    seamstressesCache
      .map((s) => {
        ...
        return `
          <option value="${escapeHtml(s.seamstress_id)}">
            ${escapeHtml(s.name || "")}
  ...
  orderIdField.innerHTML =
    ...
    ordersCache
      .map((o) => {
        ...
        const label =
          `#${o.order_id} - ${customerName || "Customer"} - ${o.occasion_type || "Occasion"} - ${o.dress_name || "No dress"}`;
...
function renderOrderSummary() {
  ...
  if (!urlOrderId) {
    orderSummary.innerHTML = `
      ...
      <div class="small-muted">
        Assign seamstresses to customer orders and track tailoring progress.
      </div>
    `;
  ...
  orderSummary.innerHTML = `
    <div>
      <strong>Selected Order #${escapeHtml(order.order_id)}</strong>
    </div>
...
function renderAssignments(rows) {
  ...
  assignmentsTbody.innerHTML = rows
    .map(
      (a) => `
        <tr>
          <td>${escapeHtml(a.assignment_id)}</td>

          <td>#${escapeHtml(a.order_id)}</td>

          <td>
            ${escapeHtml(a.first_name || a.customer_name || "")}
            ${escapeHtml(a.last_name || "")}
          </td>

          <td>
            ${escapeHtml(a.occasion_type || "-")}
            <div class="small-muted">
              ${escapeHtml(a.order_type || "")}
            </div>
          </td>
          ...
          <td>
            ${escapeHtml(a.name || a.seamstress_name || "-")}
            <div class="small-muted">
              ${escapeHtml(a.phone || "")}
            </div>
          </td>
          ...
          <td>
            <span class="badge-soft px-3 py-2 rounded-pill">
              ${escapeHtml(a.assignment_status || "Pending")}
            </span>
          </td>

          <td>${escapeHtml(dateOnly(a.due_date) || "-")}</td>

          <td>${escapeHtml(a.assignment_notes || a.notes || "-")}</td>

          <td>
            <div class="d-flex gap-2 flex-wrap">
              <button
                class="btn btn-sm btn-outline-secondary"
                onclick="editAssignment(${Number(a.assignment_id)})"
              >
                Edit
              </button>
              ...
              <button
                ...
                onclick="deleteAssignment(${Number(a.assignment_id)})"
              >
                Delete
              </button>
            </div>
...
window.deleteSeamstress = async function (id) {
  ...
  try {
    await fetchJson(`${SEAMSTRESSES_ENDPOINT}/${id}`, {
      method: "DELETE",
    });
...
window.editAssignment = function (id) {
  ...
  assignmentStatus.value = a.assignment_status || "Pending";
  dueDate.value = dateOnly(a.due_date);
...
window.deleteAssignment = async function (id) {
  ...
  try {
    await fetchJson(`${ASSIGNMENTS_ENDPOINT}/${id}`, {
      method: "DELETE",
    });
...
assignmentForm?.addEventListener("submit", async (e) => {
  ...
  if (!seamstressSelect.value) {
    alert("Please select a seamstress");
    return;
  }
  ...
  const data = {
    order_id: Number(orderIdField.value),
    seamstress_id: Number(seamstressSelect.value),
    task_type: taskType.value,
    assignment_status: assignmentStatus.value || "Pending",
