...
const returnDateWrap =
  ...
  returnDateInput?.closest(".col-md-4") ||
  returnDateInput?.closest(".col-md-6") ||
  returnDateInput?.parentElement;
...
if (apiUrlText) apiUrlText.textContent = "/api/orders";
...
async function loadCustomers() {
  ...
  customerSelect.innerHTML =
    ...
    customers.map((c) => {
      ...
      return `<option value="${c.customer_id}">${name || `Customer #${c.customer_id}`}</option>`;
    }).join("");
}
...
async function loadDresses() {
  ...
  dressSelect.innerHTML =
    ...
    dresses.map((d) => {
      const name = d.dress_name || `Dress #${d.dress_id}`;
      return `
        <option
          value="${d.dress_id}"
...
function renderOrders() {
  ...
  ordersTbody.innerHTML = orders.map((o) => `
    <tr>
      <td>${o.order_id}</td>
      <td>${text(o.customer_name || `${o.first_name || ""} ${o.last_name || ""}`.trim())}</td>
      ...
      <td>${text(o.status)}</td>
      <td>
        <button class="btn btn-sm btn-outline-primary me-1" onclick="editOrder(${o.order_id})">Edit</button>
...
function getSelectedDress() {
  ...
  return dresses.find((d) => Number(d.dress_id) === dressId) || null;
}

function getPriceForSelectedDress() {
  const dress = getSelectedDress();
  if (!dress) return "";
...
function autofillTotalPrice(force = false) {
  const suggestedPrice = getPriceForSelectedDress();

  if (suggestedPrice === "" || suggestedPrice === null || suggestedPrice === undefined) {
  ...
  if (force || !totalPriceInput.value || priceWasAutoFilled) {
    totalPriceInput.value = Number(suggestedPrice).toFixed(2);
    priceWasAutoFilled = true;
  }
}

function toggleReturnDate() {
  const isRental = orderTypeInput.value === "rental";

  if (returnDateWrap) {
    returnDateWrap.style.display = isRental ? "" : "none";
  }

  if (!isRental && returnDateInput) {
...
window.editOrder = function (id) {
  const order = orders.find((o) => Number(o.order_id) === Number(id));
  if (!order) return;

  orderIdInput.value = order.order_id;
  customerSelect.value = order.customer_id || "";
  dressSelect.value = order.dress_id || "";
  orderTypeInput.value = order.order_type || "sale";
  occasionTypeInput.value = order.occasion_type || "";
  orderDateInput.value = dateOnly(order.order_date);
  returnDateInput.value = dateOnly(order.return_date);
  totalPriceInput.value = order.total_price ?? "";
  statusInput.value = order.status || "in_progress";
...
orderForm?.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    dress_id: dressSelect.value ? Number(dressSelect.value) : null,
    order_type: orderTypeInput.value,
    occasion_type: occasionTypeInput.value.trim() || null,
