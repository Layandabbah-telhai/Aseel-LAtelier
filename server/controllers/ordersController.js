...
function validateOrderInput(data) {
  ...
  if (!Number.isFinite(data.dress_id)) {
    return "dress_id is required";
  }
  ...
  if (
    data.return_date &&
    data.return_date < data.order_date
  ) {
...
class OrdersController {
  ...
  async updatePayment(req, res) {
    try {
      ...
      const updated = await this.model.updatePayment(
        paymentId,
        data
      );
