class DressesModel {
  ...
  async create({
    ...
    try {
      const imagesToSave = image_urls.length
        ? image_urls
        : firstImage
        ? [firstImage]
        : [];
      ...
      if (imagesToSave.length) {
        const values = imagesToSave.map((url, index) => [
          dressId,
          url,
          index,
        ]);
  ...
  async update(
    ...
    const firstImage = image_urls.length
      ? image_urls[0]
      : image_url || null;
  ...
  async remove(dress_id) {
    ...
    try {
      ...
      /*
        If a dress is connected to old orders,
        we do not delete those orders.
        We only detach the dress from them,
        then the dress can be deleted safely.
      */
      await connection
        .query(
          `
